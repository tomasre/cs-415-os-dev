...
// NEW Register process now contains a Manual Property that takes a string which will be displayed
  ...
  var userMan = " [User]; if not already a user, will add to list and prompt for user password";
  ...
  function useradd(options, argv) {
    ...
    async.waterfall([
      ...
      function (length, fh, fullData, callback) {
        ...
        result = search(lines, newUser);

		if(result == "User not found") {
			stdout.appendToBuffer("Adding user...");
			lines[lines.length] = newUser + ",";
...
/*
  ...
  */
  ...
  function search(substr, key) {
	...
	function useraddListener(stream) {
		...
		lines[lines.length - 1] += newPassword;
		...
		async.waterfall([
			...
			function (fullResult, recursivecallback) {
				...
				if(pipeIn == true) {
					os._internals.ps.processTable['useradd'].options.pipeIn = false;
				} else {
