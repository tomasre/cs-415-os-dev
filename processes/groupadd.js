...
// NEW Register process now contains a Manual Property that takes a string which will be displayed
  ...
  os.bin.groupadd = groupadd;
  os.ps.register('groupadd', groupadd, {stdout: true}, userMan);

  function groupadd(options, argv) {
		...
		async.waterfall([
			...
			function (length, fh, fullData, callback) {
				...
				for(var i = 0; i < lines.length; i++) {
					...
					if(theGroup[0] == group) {
						// If group found, add the user to the group
						theGroup.push(newUser);
				...
				async.waterfall([
					...
					function (fullResult, userFile, recursivecallback) {
						os.fs.close(userFile,function(errClose,msg){
							...
							if (error===-1) {
								console.log('groupadd: ERROR in execution. exited early');
							} else {
